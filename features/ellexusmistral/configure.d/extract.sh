...
if [[ -f $package && -f $license ]]; then
  ...
  if [[ $? -eq 0 ]]; then
       ...
       cp "`dirname "$0"`"/../resources/modulefile "$cw_ROOT/etc/modules/apps/ellexusmistral"
  cat <<EOF
******************************
...
******************************
  ...
  Then, please copy the License file and the zip archive to: 
  
  /tmp/ellexusmistral-license.lic
  /tmp/mistral_2.9.4_x86_64.zip 
